

(* Star.nb *)

VerificationTest[
    Begin["Global`"];
    ClearAll["`*"]
    ,
    Null
    ,
    TestID->"[0] Star.nb"
]

VerificationTest[
    Get["Yurie`Cluster`"]
    ,
    Null
    ,
    TestID->"[1] Star.nb"
]

VerificationTest[
    ClearAll[cl]
    ,
    Null
    ,
    TestID->"[2] Star.nb"
]

VerificationTest[
    clusterInit[{"cl", "Global`"}, {planet1, planet2, planet3, planet4}, {{a[1]}, {b[1]}, {c[1]}, {d[1]}}, {{a[0]}, {b[0]}, {c[0]}, {d[0]}}, {Join, Sort @* Join, Union, Sort @* Union}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {}, "StarData" -> Association[], "StarDefaultList" -> {}, "StarDefaultData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}]]
    ,
    TestID->"[3] Star.nb"
]

VerificationTest[
    starDefine[cl, {star1, star2}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {}, "StarDefaultData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}]]
    ,
    TestID->"[4] Star.nb"
]

VerificationTest[
    starMerge[cl, {star1}, Association[planet1 -> {test}]]; 
    starDefault[cl, {star1}]; 
    starDefine[cl, {star1}]; 
    cl["Data"]
    ,
    Quiet[Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1], test}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {star1}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1], test}, planet2 -> {b[0], b[1]}, planet3 -> {c[0], c[1]}, planet4 -> {d[0], d[1]}]]]
    ,
    {General::StarDef}
    ,
    TestID->"[5] Star.nb"
]

VerificationTest[
    starDefineReset[cl, {star1}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {star1}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1]}, planet2 -> {b[0], b[1]}, planet3 -> {c[0], c[1]}, planet4 -> {d[0], d[1]}]]
    ,
    TestID->"[6] Star.nb"
]

VerificationTest[
    starDefault[cl, {star3}]; 
    cl["Data"]
    ,
    Quiet[Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {}, "StarDefaultData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}]]]
    ,
    {General::StarUndef}
    ,
    TestID->"[7] Star.nb"
]

VerificationTest[
    starDefault[cl, {star1, star2}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {star1, star2}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1], a[1]}, planet2 -> {b[0], b[1], b[1]}, planet3 -> {c[0], c[1]}, planet4 -> {d[0], d[1]}]]
    ,
    TestID->"[8] Star.nb"
]

VerificationTest[
    starMerge[cl, {star1, star2}, Association[planet1 -> {a[11]}, planet2 -> {b[11]}, planet3 -> {c[11]}, planet4 -> {d[11]}]]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1], b[11]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}], star2 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1], b[11]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}]], "StarDefaultList" -> {star1, star2}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1], a[11], a[1], a[11]}, planet2 -> {b[0], b[1], b[1], b[11], b[11]}, planet3 -> {c[0], c[1], c[11]}, planet4 -> {d[0], d[1], d[11]}]]
    ,
    TestID->"[9] Star.nb"
]

VerificationTest[
    starMerge[cl, {star3}, Association[planet1 -> {a[11]}, planet2 -> {b[11]}, planet3 -> {c[11]}, planet4 -> {d[11]}]]; 
    cl["Data"]
    ,
    Quiet[Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1], b[11]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}], star2 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1], b[11]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}]], "StarDefaultList" -> {star1, star2}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1], a[11], a[1], a[11]}, planet2 -> {b[0], b[1], b[1], b[11], b[11]}, planet3 -> {c[0], c[1], c[11]}, planet4 -> {d[0], d[1], d[11]}]]]
    ,
    {General::StarUndef}
    ,
    TestID->"[10] Star.nb"
]

VerificationTest[
    starChange[cl, {star1, star2}, Association[planet2 -> {b[11]}], {planet2 -> Complement}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}], star2 -> Association[planet1 -> {a[1], a[11]}, planet2 -> {b[1]}, planet3 -> {c[1], c[11]}, planet4 -> {d[1], d[11]}]], "StarDefaultList" -> {star1, star2}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1], a[11], a[1], a[11]}, planet2 -> {b[0], b[1], b[1]}, planet3 -> {c[0], c[1], c[11]}, planet4 -> {d[0], d[1], d[11]}]]
    ,
    TestID->"[11] Star.nb"
]

VerificationTest[
    starDefault[cl, {star1}]; 
    starReset[cl, {star1, star2}]; 
    cl["Data"]
    ,
    Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {star1, star2}, "StarData" -> Association[star1 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], star2 -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}]], "StarDefaultList" -> {star1}, "StarDefaultData" -> Association[planet1 -> {a[0], a[1]}, planet2 -> {b[0], b[1]}, planet3 -> {c[0], c[1]}, planet4 -> {d[0], d[1]}]]
    ,
    TestID->"[12] Star.nb"
]

VerificationTest[
    starUnset[cl, {star1, star2}]; 
    cl["Data"]
    ,
    Quiet[Association["ClusterName" -> "cl", "PlanetList" -> {planet1, planet2, planet3, planet4}, "PlanetCommonData" -> Association[planet1 -> {a[1]}, planet2 -> {b[1]}, planet3 -> {c[1]}, planet4 -> {d[1]}], "PlanetExtraData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}], "PlanetMergeData" -> Association[planet1 -> Join, planet2 -> Sort @* Join, planet3 -> Union, planet4 -> Sort @* Union], "StarList" -> {}, "StarData" -> Association[], "StarDefaultList" -> {}, "StarDefaultData" -> Association[planet1 -> {a[0]}, planet2 -> {b[0]}, planet3 -> {c[0]}, planet4 -> {d[0]}]]]
    ,
    {General::StarRemoveDefault}
    ,
    TestID->"[13] Star.nb"
]

VerificationTest[
    ClearAll[cl]
    ,
    Null
    ,
    TestID->"[14] Star.nb"
]

VerificationTest[
    ClearAll["`*"];
    End[]
    ,
    "Global`"
    ,
    TestID->"[∞] Star.nb"
]